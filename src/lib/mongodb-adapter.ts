...
import type { Adapter, AdapterUser as BaseAdapterUser, AdapterAccount, AdapterSession, VerificationToken } from '@auth/core/adapters';
import type { MongoClient } from 'mongodb';

import { ProfileExtension } from './types';

const defaultExtension: ProfileExtension = {
    majors: [],
    concentrations: [],
    minors: [],
...
// Extend AdapterUser to include additional user data
...
export function MongoDBAdapter(
     ...
     */
    client: MongoClient | Promise<MongoClient> | (() => MongoClient | Promise<MongoClient>),
    ...
    const getDb = async () => {
        const _client: MongoClient = await (typeof client === 'function' ? client() : client);
    ...
    return {
        ...
        async getUser(id) {
            ...
            if (!user) return null;
            return { ...from<AdapterUser>(user), extension: user.extension || defaultExtension };
        },
